...
export class ImapEmailClient extends EventEmitter {
  ...
  async openBox(boxName: string = 'INBOX', readOnly: boolean = true): Promise<any> {
    return new Promise((resolve, reject) => {
      if (!this.imap || !this.connected) {
        reject(new Error('Not connected to IMAP server'));
        return;
  ...
  async deleteEmails(uids: number[], boxName: string = 'INBOX'): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.imap || !this.connected) {
        reject(new Error('Not connected to IMAP server'));
        return;
  ...
  async moveEmailsToTrash(uids: number[], trashFolder: string = 'Trash', boxName: string = 'INBOX'): Promise<void> {
    return new Promise((resolve, reject) => {
      if (!this.imap || !this.connected) {
        reject(new Error('Not connected to IMAP server'));
        return;
  ...
  async searchEmails(criteria: any[] = ['ALL'], boxName: string = 'INBOX'): Promise<number[]> {
    return new Promise((resolve, reject) => {
      if (!this.imap || !this.connected) {
        reject(new Error('Not connected to IMAP server'));
        return;
  ...
  async fetchEmails(uids: number[], boxName: string = 'INBOX'): Promise<EmailMessage[]> {
    return new Promise((resolve, reject) => {
      if (!this.imap || !this.connected) {
        reject(new Error('Not connected to IMAP server'));
        return;
  ...
  async getRecentEmails(count: number = 10, boxName: string = 'INBOX'): Promise<EmailMessage[]> {
    try {
