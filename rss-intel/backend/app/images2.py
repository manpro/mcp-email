...
class ImageExtractor:
    ...
    async def extract_primary_image(
        ...
        if not all_candidates or max(c.confidence for c in all_candidates) < 0.8:
        ...
        if not all_candidates or max(c.confidence for c in all_candidates) < 0.7:
            try:
        ...
        if (settings.image_playwright_enabled and 
            (not all_candidates or max(c.confidence for c in all_candidates) < 0.7)):
            try:
